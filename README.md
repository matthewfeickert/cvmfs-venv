...
[feickert@lxplus924 ~]$ curl -sSfL https://raw.githubusercontent.com/matthewfeickert/cvmfs-venv/v0.0.8/cvmfs-venv.sh -o ~/.local/bin/cvmfs-venv && chmod +x ~/.local/bin/cvmfs-venv
...
`cvmfs-venv` has no dependencies beyond the ones it aims to extend: A Linux operating system that has CVMFS installed on it with a Python 3.8+ runtime with a functioning [`venv` module][venv docs].

A full listing of all programs used outside of Bash shell builtins are:
* `cat`
* `readlink`
* Python 3.8+ with `pip` (the environment's own Python adds the hooks to its `bin/activate`)
...
### Side effects in the shell that sources cvmfs-venv

* `PIP_REQUIRE_VIRTUALENV=true` is exported, so for the rest of the session `pip` refuses to install anything unless a virtual environment is active.
* If `uv` has to be installed, its directory is appended to `PATH` for the session only. The installer is not allowed to edit shell start-up files; add the directory (`~/.local/bin` by default) to `PATH` yourself to use `uv` in new shells. `uv self update` upgrades it later.
...
@software{cvmfs-venv,
  ...
  title = "{cvmfs-venv: v0.0.8}",
  version = {0.0.8},
  year = {2026},
  doi = {10.5281/zenodo.7751033},
  url = {https://doi.org/10.5281/zenodo.7751033},
  note = {https://github.com/matthewfeickert/cvmfs-venv/releases/tag/v0.0.8}
}
```
